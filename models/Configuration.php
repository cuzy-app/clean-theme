...
class Configuration extends Model
{
    ...
    public const MENU_STYLE_SOFT = 'soft';
    ...
    public static function getMenuStyleOptions()
    {
        return [
            ...
            self::MENU_STYLE_SOFT => Yii::t('CleanThemeModule.config', 'Soft highlight background with primary color text for active items'),
        ];
    }
    ...
    /**
     * @param Theme|null $rebuildTheme if provided, the CSS of this theme is rebuilt (e.g. right after a theme
     *                                 activation, when `Yii::$app->view->theme` is not refreshed yet);
     *                                 otherwise the CSS of the active theme is rebuilt if it is based on the Clean Theme
     * @throws Exception
     */
    public function generateScssRootFile(?Theme $rebuildTheme = null): void
    {
        ...
        // Rebuild CSS
        if ($rebuildTheme !== null || Module::isThemeBasedActive()) {
            $buildResult = ThemeHelper::buildCss($rebuildTheme);
            if ($buildResult !== true) {
    ...
    {
        $fonts = [];
        if (!in_array($this->fontFamily, Module::$locallyHostedFonts, true)) {
            $fonts[] = $this->fontFamily;
        }
        if (
            !in_array($this->headingFontFamily, Module::$locallyHostedFonts, true)
